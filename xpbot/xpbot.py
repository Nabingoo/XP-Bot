...
async def on_message(message):
    ...
    #check if message.author is in database
        #if not in database, add them
        #give xp 
        # 
        # if in database
        # check if message.created_at - last sent message timestamp = less than 2 minutes
        # if older than 2 minutes, give xp, overwrite old timestamp 
        # if younger than 2 minutes do nothing       
        # 
    ...
    if v_db_discordID ==  None:
        ...
        cursor = conn.execute('''update UserInfo set xpamount = 1 where DiscordID=?''',[v_mem_disc_id])
        #cursor = conn.execute('''INSERT INTO UserInfo (DiscordID,xpamount,lastmessage) VALUES (?,?,?)''',(v_mem_disc_id, v_db_xp,message.created_at))
        conn.commit()        
    else:
        ...
        timereq = float(5.0)
        if v_diff >= timereq:
            cursor = conn.execute('''update UserInfo set lastmessage=? , xpamount = xpamount + 1  where DiscordID=?''',[message.created_at,v_mem_disc_id])
            conn.commit()
