...
async def level(ctx):
    ...
    for row in cursor:
        v_db_level = row[0]
        print(v_db_level)
    cursor = conn.execute('''select xpamount from UserInfo where DiscordID = ?''',[ctx.author.id])
    for row in cursor:
        v_db_currxp = row[0]
        print(v_db_currxp)
    ...
    for row in cursor:
        v_db_needxp = row[0]
        print(v_db_needxp)
    embed=discord.Embed(title=v_db_level, description= 'Current XP= ' + str(v_db_currxp) + '/' + str(v_db_needxp), color=discord.Color.dark_grey())
